		...
		onClick: function(){
			$(this).find('>a').each(function(){
				if ( this.href )
				{
					...
					if(_r)
					{
						getView(this.href,_r,$(this).text());
						return false;
					}else
					{
						window.location = this.href;
	    ...
	    return this.each(function()
		{
			...
			for ( var j = 0; j < liElems.length; j++ )
			{
				if ( getOneChild(getOneChild(getOneChild(liElems[j], 'DIV'), 'UL'), 'LI') ) // >div>ul>li
				{
					...
					if(settings.subArrowSrc)
					$(liElems[j]).append("<img src='"+settings.subArrowSrc+"' class='liArrow2'>");
					
				}else
				{
					$(liElems[j]).click(ascMeWathDo);
				}
			}
