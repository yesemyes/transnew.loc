<?
class feTechnical extends front 
{
	...
	function __construct($init)
	{
		...
		**/
        ...
        //$this->headScripts [] = "/scripts/ui.accordion.js";
...
{
        $db = db::getInstance();
       $this->citys = $db->getArray('regions','*','lng_id = ' . $this->defLng ['id'] . " AND `active` AND `pid` = 0 ",'','position','');
           
            foreach($this->citys as &$r)
            {
                
                $r['sub_region'] = $db->getArray('regions','*','lng_id = ' . $this->defLng ['id'] . " AND `pid` = '{$r['id']}'",'','position','');
           ...
           public function getAccordionContent()
            {
                ...
                $this->technical = $db->getArray( 'technicalinspection', '*', " `lng_id` = '{$this->defLng ['id']}' AND `region` = '{$regionId}' AND `active` ", '', '','1' );
