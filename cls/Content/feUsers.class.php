...
class feUsers extends front {
	...
	function viewSuccessNewPasswordSend()
    {
        ...
        if (empty ( $row )) {
                ...
                base::redirect ( HelperFunction::link ( array ('users', 'recoveredpassword' ) ) );
    		}
      
      
        
    }
    function viewRecoveredPassword(){}
	...
	function viewDeleteOffer() {
		...
		base::redirect ( HelperFunction::link (array( 'users', 'mycars' ) ));
	}
	function viewRegistration() {
		...
		if ($this->method == 'POST') {
			$InsertData ['main'] ['login'] = trim ( $db->escape($this->post ['data_login'] ));
			$InsertData ['main'] ['name'] = trim ( $db->escape($this->post ['data_name'] ));
			$InsertData ['main'] ['email'] = trim ( $db->escape($this->post ['data_email'] ));
		...
		/*	if ($this->post ['user_phone1_code'] && $this->post ['user_phone1'])
			...
			$InsertData ['main'] ['data_currentcar_brand'] = $this->post ['data_currentcar_brand'] ? $db->escape($this->post ['data_currentcar_brand']) : 0;
			$InsertData ['main'] ['data_currentcar_brand_model'] = $this->post ['data_currentcar_brand_model'] ? $db->escape($this->post ['data_currentcar_brand_model']) : 0;
			...
			if ($sucsess) {
				...
				$this->languages = $db->getArray ( 'languages', '*', 'active', '', 'position', '', 'id' );
				foreach ( $registration_email_content as $key => $row ) {
					...
					$row ['content'] = str_replace ( '{url}', 'http://' . $_SERVER ['HTTP_HOST'] . "" .  '/'.$this->languages[$key]['code'].'/users/confirm.html'  . "?code={$confirm_code}", $row ['content'] );
	...
	function viewLogin() {
        ...
        if ($this->method == 'POST') {
			...
			$login = $db->escape($this->post ['user_login']);
			$password = $db->escape($this->post ['user_password']);
			...
			if(isset($_POST['token'])){
               ...
               // die('<pre>'.print_r($user,1));
				               $login = uniqid('soc',true);
				$q  = "INSERT INTO  `siteusers` (`login`,`regdate`,`active`,`confirmed`,`email`,`network`,`name`,`identity`,`photo`,`photo_big`) values('{$login}',NOW(),'1','1','{$user['email']}','{$user['network']}',concat('{$user['first_name']} ', ' {$user['last_name']}'),'{$user['identity']}','{$user['photo']}','{$user['photo_big']}') ON DUPLICATE KEY UPDATE `lastactivity`=NOW(),`name`='{$user['first_name']} {$user['last_name']}',`photo`='{$user['photo']}',`photo_big`='{$user['photo_big']}',`network`='{$user['network']}',`identity`='{$user['identity']}'";
	...
	function viewForgot() {
		...
		if ($this->method == 'POST') {
			...
			foreach ( $forgot_email_content as $key => $rowt ) {
                ...
                $rowt ['content'] = str_replace ( '{url}', 'http://' . $_SERVER ['HTTP_HOST'] . "" .  '/'.$this->languages[$key]['code'].'/users/successnewpasswordsend.html'  . "?code={$confirm_code}", $rowt ['content'] );
			...
			base::redirect ( HelperFunction::link ( array ('users', 'new-password-send' ) ) );
	...
	function viewChangeprofildata() {
		...
		if ($this->method == "POST") {
            ...
            $row = $db->getRow ( 'siteusers', '*', " siteusers.id='{$row['id']}'");
			
            
		    $row['data_currentcar_brand'] = $db->getRow( 'brandmodel', 'label', "`id` = '{$row['data_currentcar_brand']}'" );
            $row['data_currentcar_brand_model'] = $db->getRow( 'brandmodel', 'label', "`id` = '{$row['data_currentcar_brand_model']}'" );
			$_SESSION ['siteusers'] = $row;
			base::redirect ( HelperFunction::link ( 'changeprofildata', 'sucsess' ) );
	...
	function getMyCars() {
		...
		$pageA = end($this->path_params);
		$page = isset ( $this->pages [$pageA] ) ? $this->pages [$pageA] : 1;
        
		$page = $page < 1 ? 1 : $page;
		$limit = $this->config ['mycars_limit'];
		$limit_str = " LIMIT " . (($page - 1) * $this->config ['mycars_limit']) . ", {$this->config['mycars_limit']} ";
		...
		$query = " SELECT 		  offers.id,
                                  ...
                                  offers.filed_price as  filed_price,
                                  offers.filed_credit as  credit,
                                  offers.filed_interchange as  interchange,
				                  offers.filed_release_date_month as month,  
				                  offers.filed_currency as currency_id,
                                  offers.price_usd as price_usd,
		...
		if (isset ( $this->query ['order_by'] )) {
			...
			if (isset ( $this->query ['direction'] ) && in_array ( $this->query ['direction'], array ('asc', 'desc' ) )) {
			     $ord = $this->query ['direction'];
				$order .= " {$ord}" ;
			
			}
		}
		$found = $db->queryFetch ( $query . ($order  != '' ? "ORDER BY '{$order}'":''), $limit_str);
	...
	function viewAddoffer() {
	   ...
	   $this->headStyleSheets [] = "/css/combo.css";
        ...
        $this->headScripts [] = "/scripts/jquery.mask.js";
		...
		if ($this->method == 'POST' && isset ( $this->post ['addThisOffer'] )) {
            ...
            $req ['filed_gearbox'] = intval($this->post['main']['filed_gearbox']);
            ...
            if(empty($this->errors))
            {
                ...
                $main ['filed_contract'] = $main ['filed_contract'] ? $main ['filed_contract'] : 0;
                ...
                $main ['filed_credit'] = $main ['filed_credit'] ? $main ['filed_credit'] : 0;
                $main ['filed_interchange'] = $main ['filed_interchange'] ? $main ['filed_interchange'] : 0;
                ...
                $main ['filed_price'] = $main ['filed_price'] ? str_replace('.','',$main ['filed_price']) : 0;
                ...
                $main ['filed_other_city'] = $main ['filed_other_city'] ? $db->escape($main ['filed_other_city']) : 0;
    			$main ['active'] = 1;
    			...
    			if ($main ['filed_currency'] == '3'){
                    ...
                    $priceUSD = "UPDATE  `offers` SET `price_usd`=`filed_price` WHERE id=LAST_INSERT_ID()";
                    }
			    elseif ($main ['filed_currency'] == '4'){
    				$service_price = " UPDATE  `offers` SET `service_price`=`filed_price` *{$rates['EUR']} WHERE id=LAST_INSERT_ID()";
                    $priceUSD = "UPDATE  `offers` SET `price_usd`=`service_price` /{$rates['USD']}  WHERE id=LAST_INSERT_ID()";
                    }
...
$this->mainTpl = "users/addoffer.tpl.html";
	...
	function viewEditoffer() {
		...
		if ($this->method == 'POST' && isset ( $this->post ['addThisOffer'] )) {
            ...
            if ($main ['filed_currency'] == '3'){
				$service_price = " UPDATE  `offers` SET `service_price`=`filed_price` *{$rates['USD']} WHERE id={$this->post['offer_id']}";
                $priceUSD = "UPDATE  `offers` SET `price_usd`=`filed_price` WHERE id={$this->post['offer_id']}";
                }
			elseif ($main ['filed_currency'] == '4'){
                $service_price = " UPDATE  `offers` SET `service_price`=`filed_price`*{$rates['EUR']} WHERE id={$this->post['offer_id']}";
                $priceUSD = "UPDATE  `offers` SET `price_usd`=`service_price` /{$rates['USD']}  WHERE id={$this->post['offer_id']}";
			}
			elseif ($main ['filed_currency'] == '5'){
				$service_price = " UPDATE  `offers` SET `service_price`=`filed_price` WHERE id={$this->post['offer_id']}";
                $priceUSD = "UPDATE  `offers` SET `price_usd`=`service_price` /{$rates['USD']}  WHERE id={$this->post['offer_id']}";
                }
			$db->query ( $service_price );
            $db->query ( $priceUSD );
