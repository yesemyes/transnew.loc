<?
class FeRoadpolice extends front {
	function __construct($init) {
        ...
        public function RoadpoliceAction()
        {
		...
		$db = db::getInstance ();
		$parent = $db->escape($this->path_params[1]);
        
        $this->curr = $db->getRow("menu","*","`active` = '1' AND `lng_id` = '{$this->defLng['id']}' AND `alias` = '{$parent}'");
        if(!$this->curr)
            {
                $this->errorPage();
            }
